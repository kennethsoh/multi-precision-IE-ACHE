...
int main() {
	...
	LweSample* ciphertextnegative1 = new_gate_bootstrapping_ciphertext_array(32, params);
	LweSample* ciphertextbit1 = new_gate_bootstrapping_ciphertext_array(32, params);

	LweSample* ciphertextnegative2 = new_gate_bootstrapping_ciphertext_array(32, params);
	LweSample* ciphertextbit2 = new_gate_bootstrapping_ciphertext_array(32, params);
	...
	for (int i = 0; i<32; i++) // line0
		import_gate_bootstrapping_ciphertext_fromFile(cloud_data, &ciphertextnegative1[i], params);
	...
	add(ciphertextnegative, carry1, ciphertextnegative1, ciphertextnegative2, ciphertextcarry1, 32, bk);
	...
	if (int_bit1 >= int_bit2) {
		int_bit = int_bit1;
		for (int i = 0; i<32; i++)
			export_gate_bootstrapping_ciphertext_toFile(answer_data, &ciphertextbit1[i], params);
    			std::cout << int_bit << " bit computation" << "\n";
	}
	else{
		int_bit = int_bit2;
		for (int i = 0; i<32; i++)
			export_gate_bootstrapping_ciphertext_toFile(answer_data, &ciphertextbit2[i], params);
    			std::cout << int_bit << " bit computation" << "\n";
	}
	...
	// A+B, [(-A)+(-B)], A-(-B), (-A)-(B)
    	if ((int_op == 1 && (int_negative != 1 && int_negative != 2 )) || (int_op == 2 && (int_negative == 1 || int_negative == 2))) {
			...
			//32 Bit Addition
			if (int_bit == 32) // int_bit refers to bit size. If bit size is 32:
			{
				...
				//DONT FORGET TO DELETE THE BOTH KEYS AT THE END OF EVERYTHING
				
			}
			
			//64 Bit Addition
			if (int_bit == 64) // If bit size is 64:
			{
		...
		//if the operation is subtract OR (the operation is addition AND either one of the values are negative) 
		// A-B, A+(-B), (-A)+B
